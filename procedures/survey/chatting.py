...
from procedures.survey.prompts import (
    product_prompt,
    start_conversation,
...
async def finalize_output(id: str):
    data = await db.registrations.find_one({"_id": validate_object_id(id)})
    prompts = data["conversations"]
    prompts.append({"role": "developer", "content": finalize_conversation()})
...
async def initiate_conversation(lang: str, email: str, ip: str):
    ...
    prompts = [
        {"role": "developer", "content": product_prompt()},
        {"role": "user", "content": start_conversation(lang, email, ip)},
    ]
    ...
    result = client.chat.completions.create(model="gpt-4o", messages=prompts)
...
async def develop_conversation(id: str, message: str):
    ...
    prompts = data["conversations"]
    prompts.append({"role": "user", "content": continue_conversation(message)})
    await db.registrations.update_one(
        {"_id": validate_object_id(id)},
        {
            "$push": {
